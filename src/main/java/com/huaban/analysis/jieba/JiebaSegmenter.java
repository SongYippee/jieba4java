...
import com.huaban.analysis.jieba.word.PartOfSpeechTagging;


public class JiebaSegmenter {
    private static WordDictionary wordDict = WordDictionary.getInstance();
    private static FinalSeg finalSeg = FinalSeg.getInstance();
    private static PartOfSpeechTagging partOfSpeechTagging = PartOfSpeechTagging.getInstance();
    ...
    public List<SegToken> process(String paragraph, SegMode mode, boolean tagging) {
        List<SegToken> tokens = new ArrayList<SegToken>();
        StringBuilder sb = new StringBuilder();
        int offset = 0;
        for (int i = 0; i < paragraph.length(); ++i) {
            ...
            if (CharacterUtil.ccFind(ch))
                ...
                if (sb.length() > 0) {
                    ...
                    if (mode == SegMode.SEARCH) {
                        for (String word : sentenceProcess(sb.toString())) {
                            if (tagging) {
                                tokens.add(new SegToken(word, offset, offset += word.length(), partOfSpeechTagging.process(word)));
                            } else {
                                tokens.add(new SegToken(word, offset, offset += word.length()));
                            }
                        }
                        ...
                        for (String token : sentenceProcess(sb.toString())) {
                            if (token.length() > 2) {
                                ...
                                for (; j < token.length() - 1; ++j) {
                                    ...
                                    if (wordDict.containsWord(gram2))
                                        if (tagging) {
                                            tokens.add(new SegToken(gram2, offset + j, offset + j + 2,
                                                    partOfSpeechTagging.process(gram2)));
                                        } else {
                                            tokens.add(new SegToken(gram2, offset + j, offset + j + 2));
                                        }
                                }
                            ...
                            if (token.length() > 3) {
                                ...
                                for (; j < token.length() - 2; ++j) {
                                    ...
                                    if (wordDict.containsWord(gram3)) {
                                        if (tagging) {
                                            tokens.add(new SegToken(gram3, offset + j, offset + j + 3,
                                                    partOfSpeechTagging.process(gram3)));
                                        } else {
                                            tokens.add(new SegToken(gram3, offset + j, offset + j + 3));
                                        }
                                    }
                            ...
                            if (tagging) {
                                tokens.add(new SegToken(token, offset, offset += token.length(), partOfSpeechTagging.process(token)));
                            } else {
                                tokens.add(new SegToken(token, offset, offset += token.length()));
                            }
                        }
                ...
                if(tagging){
                    tokens.add(new SegToken(paragraph.substring(i, i + 1), offset, ++offset, partOfSpeechTagging.process(paragraph.substring(i, i + 1))));
                } else {
                    tokens.add(new SegToken(paragraph.substring(i, i + 1), offset, ++offset));
                }
            }
                ...
                for (String token : sentenceProcess(sb.toString())) {
                    if (token.length() > 2) {
                        ...
                        for (; j < token.length() - 1; ++j) {
                            ...
                            if (wordDict.containsWord(gram2)) {
                                if(tagging){
                                    tokens.add(new SegToken(gram2, offset + j, offset + j + 2, partOfSpeechTagging.process(gram2)));
                                }else {
                                    tokens.add(new SegToken(gram2, offset + j, offset + j + 2));
                                }
                            }
                    ...
                    if (token.length() > 3) {
                        ...
                        for (; j < token.length() - 2; ++j) {
                            ...
                            if (wordDict.containsWord(gram3)) {
                                if(tagging) {
                                    tokens.add(new SegToken(gram3, offset + j, offset + j + 3, partOfSpeechTagging.process(gram3)));
                                }else{
                                    tokens.add(new SegToken(gram3, offset + j, offset + j + 3));
                                }
                            }
